...
V3_pu = V4_pu + I3_pu * 1j * x_T2pu
V3_pu = pu_functions.XFMR_phase(V3_pu, "Δ-Y")
print(f"V3_pu = {round(np.abs(V3_pu),3)} < {round(np.angle(V3_pu, deg=True),3)}° pu")

V2_pu = pu_functions.calc_vpu("V2_pu", V3_pu, I3_pu, Z_Ln1pu)

V1_pu = V3_pu + I3_pu * 1j * x_T1pu
V1_pu = pu_functions.XFMR_phase(V1_pu, "Y-Δ")
print(f"V1_pu = {round(np.abs(V1_pu),3)} < {round(np.angle(V1_pu, deg=True),3)}° pu")
...
V4_actual = pu_functions.calc_actualv("V4",12.47e3,V4_pu)
...
# ------------------ 6 ---------------------------
# reflection on pf correction (95% to 105% of rated values)

# Without the pf correction solution, the voltage level errors (V6 to V1) are 3.8%, 3.1%, 7.2%, 12.4%, 13.7%, and 13.2%

# with correction = 0.5, the voltage level errors (V6 to V1) are 3.8%, 3.7%, 6.9%, 9.9%, 9.4%, and 8.9%

# with correction = 1, the voltage level errors (V6 to V1) are 3.8%, 3.8%, 6.1%, 6.7%, 4.4%, and 3.9%

# the pf correction corrects the voltage level to just over 95%/105% the rated voltage level.
...
print('\nchanging base values:')
S_base, V_base1, V_base2, V_base4, V_base6 = pu_functions.change_base_vals(50e6,10e3,100e3,30e3,12e3)
...
# calculate actual voltages
print ('\n4.')
V6_actual = pu_functions.calc_actualv("V6",V_base6,V6_pu)
V5_actual = pu_functions.calc_actualv("V5",V_base4,V5_pu)
V4_actual = pu_functions.calc_actualv("V4",V_base4,V4_pu)
V3_actual = pu_functions.calc_actualv("V3",V_base2,V3_pu)
V2_actual = pu_functions.calc_actualv("V2",V_base2,V2_pu)
V1_actual = pu_functions.calc_actualv("V1",V_base1,V1_pu)
...
# reflection on changing base values:
# the base values stay the same but the per unit values change